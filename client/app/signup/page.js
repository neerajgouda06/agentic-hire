...
export default function SignupPage() {
  ...
  // Prevent logged-in users from seeing signup page or going back to it
  useEffect(() => {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50 dark:bg-[#0b0f19] text-slate-900 dark:text-slate-100 selection:bg-indigo-600 selection:text-white relative">
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center space-y-1.5">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Create Recruiter Account</p>
        </div>
        ...
        <Card className="bg-white dark:bg-slate-900/90 border border-slate-200/90 dark:border-slate-800 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none">
          <CardHeader className="border-b border-slate-100 dark:border-slate-800/80 pb-4">
            <CardTitle className="text-lg font-bold text-slate-900 dark:text-white">Get Started</CardTitle>
            <CardDescription className="text-xs text-slate-500 dark:text-slate-400">Create your account to launch AI hiring pipelines</CardDescription>
          </CardHeader>
          <CardContent className="pt-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                />
                {errors.name && <p className="text-xs text-rose-600 dark:text-rose-400">{errors.name.message}</p>}
              </div>
